...
def loading(toolbarSize, commands=[], waitTime=0.2):
    """
    Returns a loading bar on the screen
    @toolbarSize The length of the toolbar (defaults 10 + length of commands list)
    @commands Enter your commands phrase by phrase in 2D lists [["cd",".."],["ls","-a"]] (default is empty)
    @waitTime The time between each '-' on the bar when commands list is empty (defaults to 0.2 seconds)
    """
    from subprocess import check_output
    from time import sleep
    # Setup toolbar
    print(f"[{' '*toolbarSize}]", flush=True, end="")
    print("\b" * (toolbarSize+1), flush=True, end="") # Return to start of line, after '['
    ...
    print("]") # This ends the progress bar
    if listlen == 0:
                ...
                while True:
                    ...
                    for character in usernameLower:           
                        if character in letters:
                            ...
                            print("You must use English letters and/or numbers")
                            exitLoop = False
                            break
                    if exitLoop:
                        break
                    del usernameLower, character, exitLoop
                # Make file
